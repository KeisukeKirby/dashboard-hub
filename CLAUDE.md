...
- サムネが無いカードだけ撮る: `python build.py --shots-new`(日次実行用。既存のサムネは撮り直さない)
- ネットワークなしで見た目だけ確認: `python build.py --offline`
- 日次更新: Windows タスク「Barefootinc Backroom refresh」→ `refresh.ps1`。main をチェックアウトしていて未コミットの変更が無いときだけ動き、生成物(index.html / artifact.html / thumbs / p)だけを main に push する。ログは `refresh.log`
...
## このプロジェクト固有の規約
- 名前・分野・説明文は `catalog.json` に手で書く。自動取得するのは HTTP 状態・更新日・サムネだけ
- カタログに無い公開プロジェクト(watchScopes の Vercel、homepage を持つ GitHub リポジトリ)はビルド時に自動で「新規(未分類)」へ載る。名前はページの `<title>`、説明は meta description。カードに「自動追加」と出る
- 自動追加されたカードの id は Vercel プロジェクト名。items へ移すときに id を変えると、サムネのファイル名も変わる
...
- ビルド時に「自動追加」と出たものは、分野と説明を付けるなら items へ移し、載せたくないなら ignore へ入れる
- 推測で書いた説明には「推定」と明記する
